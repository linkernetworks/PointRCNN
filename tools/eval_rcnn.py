        ...
        for k in range(bbox3d.shape[0]):
            ...
            print(
                ...
                % (cfg.CLASSES, alpha, img_boxes[k, 0], img_boxes[k, 1],
                   ...
                   bbox3d[k, 2], bbox3d[k, 6], sigmoid(scores[k])),
                file=f)
...
def eval_one_epoch_rpn(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        rois, roi_scores_raw = pure_model.rpn.proposal_layer(
...
def eval_one_epoch(model, dataloader, epoch_id, result_dir, logger):
    if cfg.RPN.ENABLED and not cfg.RCNN.ENABLED:
        ...
        ret_dict = eval_one_epoch_rcnn(model, dataloader, epoch_id, result_dir,
                                       logger)
    elif cfg.RPN.ENABLED and cfg.RCNN.ENABLED:
        ret_dict = eval_one_epoch_joint(model, dataloader, epoch_id,
                                        result_dir, logger)
    else:
...
def eval_single_ckpt(root_result_dir):
    ...
    load_ckpt_based_on_args(pure_model, logger)
...
def repeat_eval_ckpt(root_result_dir, ckpt_dir):
    ...
    tb_log = SummaryWriter(
        logdir=os.path.join(root_result_dir, 'tensorboard_%s' %
                            cfg.TEST.SPLIT))
    ...
    while True:
        ...
        pure_model = model.module if isinstance(
            model, torch.nn.DataParallel) else model
        train_utils.load_checkpoint(pure_model,
                                    filename=cur_ckpt)
